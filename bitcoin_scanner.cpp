...
// Base58Check decode → 20‑byte payload
...
// reservoir‑sample up to MAX_FUNDED_ADDRESSES, decode into set
std::unordered_set<std::array<unsigned char,20>> loadFunded(const std::string& path) {
  ...
  if (path.size()>3 && path.substr(path.size()-3)==".gz") {
    ...
    while (gzgets(gz, buf, BUF)) {
      ...
      if (auto t = line.find('\t'); t!=std::string::npos) line.resize(t);
      process(line);
    }
    gzclose(gz);
  } else {
...
// Base58Check encode (for logging hits)
...
// derive pubkey, hash160, match raw, log hex+WIF on hit
